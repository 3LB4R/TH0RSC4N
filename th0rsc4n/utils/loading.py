"""Animated loading screens untuk TH0RSC4N — HACKER GREEN MATRIX."""
import time
import threading
import random
...
class HackerLoading:
    ...
    def _build_display(self):
        ...
        text.append(" ██  ", style="bold bright_green")
        text.append("// CYBER INTELLIGENCE", style="bold white")
        text.append("\n\n")
...
class QuickSpinner:
    ...
    def start(self):
        self.status = console.status(f"[bold bright_green]{self.message}[/bold bright_green]", spinner="dots")
        self.status.start()

    def stop(self, success=True):
        if self.status:
            self.status.stop()
        icon = "[bold bright_green]✓[/bold bright_green]" if success else "[bold bright_red]✗[/bold bright_red]"
