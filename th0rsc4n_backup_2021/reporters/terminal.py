"""Terminal reporter — OWASP 2025 grouped output."""
...
def _owasp_color(code):
    """Get warna dari kode A01-A10."""
    if not code:
        return "white"
    prefix = code[:3]
    return OWASP_COLORS.get(prefix, "white")
...
def report(findings, target, verbose=False):
    ...
    console.print(Panel.fit(
        ...
        f"[bold cyan]OWASP :[/bold cyan] [white]Top 10 2025 Edition[/white]\n"
        f"[bold cyan]TOTAL :[/bold cyan] [white]{len(findings)} findings[/white]",
    ...
    # Sort by OWASP code (A01 → A10)
    sorted_keys = sorted(by_owasp.keys())

    for owasp_key in sorted_keys:
        ...
        # Sort per severity dalam kategori
